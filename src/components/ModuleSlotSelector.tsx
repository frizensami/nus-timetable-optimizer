...
const ModuleSlotSelector: React.FC<ModuleConstraintsProps> = ({ open, updateOpen, mod, updateModuleSlotSelections, initialSlotSelections }) => {
    ...
    function parseMod(): LessonTypeConstraints {
        ...
        grouped_lessontypes.forEach((value: any, key: string, _: any) => {
            ...
            lessons_for_lessontype.forEach((lesson: any) => classes_for_lessontype.add(lesson["classNo"]));

            // Map the class numbers to an array of select options
            const classes_array = Array.from(classes_for_lessontype).sort(function(a: any, b: any) { return a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' }) });
            const select_options = classes_array.map((classNo: any) => {
                return { key: key + "_" + classNo, text: key + " " + classNo, value: classNo }
            })
    ...
    return (
        <Modal
            ...
            <Modal.Content>
                <Modal.Description>
                    <h5>You may use this interface to select which lecture / tutorial / etc slots to restrict the optimizer to. </h5>
                                    ...
                                    return (
                                        <Form.Group key={'form-group-lessontype' + i} widths="equal">
                                            <Form.Field
                                                ...
                                                defaultValue={initialSlotSelections === undefined ? undefined : initialSlotSelections[lessonType]}
                                                placeholder='Unrestricted'
                                                fluid multiple selection search clearable
                                                label={mod?.module_code + " " + lessonType + ": only these class numbers will be considered during optimization. Leave blank for no restrictions."}
                                                width={10}
                                                onChange={(_: any, { value }: any) => updateConstraints(lessonType, value)} />
                                        </Form.Group>
                                    );
                                })}
